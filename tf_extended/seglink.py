...
def match_anchor_to_text_boxes(anchors, xs, ys):
    ...
    assert len(np.shape(anchors)) == 2 and np.shape(anchors)[
        1] == 4, "the anchors must be a tensor with shape = (num_anchors, 4)"
    assert len(np.shape(xs)) == 2 and np.shape(xs) == np.shape(ys) and np.shape(ys)[
        1] == 4, "the xs, ys must be a tensor with shape = (num_bboxes, 4)"
...
def tf_seglink_to_bbox(seg_cls_pred, link_cls_pred, seg_offsets_pred, image_shape,
    ...
    image_bboxes = tf.py_func(seglink_to_bbox,
                              [seg_scores, link_scores, seg_offsets_pred, image_shape, seg_conf_threshold,
                               link_conf_threshold],
                              tf.float32)
    return image_bboxes
